...
class StocksHistoryRepositoryImpl extends StocksHistoryRepository {
  ...
  async fetchStockData(symbol?: string): Promise<any[] | null> {
    ...
    const AlphaAPiKey = process.env.ALPHA_API_KEY
		console.log("TCL: StocksHistoryRepositoryImpl -> AlphaAPiKey", AlphaAPiKey)

    const uri = `https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=${symbol || 'AAPL'}&interval=15min&apikey=${AlphaAPiKey}`

    // const stocks = await axios.get(uri)
    const stocks = {
      data: {
        "Meta Data": {
            "1. Information": "Intraday (5min) open, high, low, close prices and volume",
            "2. Symbol": "IBM",
            "3. Last Refreshed": "2025-03-03 19:55:00",
            "4. Interval": "5min",
    ...
    const stocksNormalize = fetchStocks(stocks.data, symbol)

    return stocksNormalize
  }
